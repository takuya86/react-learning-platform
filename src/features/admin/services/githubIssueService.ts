...
function buildIssueBody(params: CreateIssueParams): string {
  const { lessonTitle, lessonSlug, hintType, followUpRate, originCount } = params;
 ...
 */
function getIssueLabels(hintType: HintType): string[] {
  return [ISSUE_LABEL_PREFIX, METRICS_LABEL, `hint:${hintType}`];
}
...
export async function createIssue(params: CreateIssueParams): Promise<IssueResult<CreatedIssue>> {
  ...
  if (isMockMode) {
    ...
    mockOpenIssues.push({
      ...issue,
      labels: getIssueLabels(hintType),
    });
  ...
  try {
    ...
    const labels = getIssueLabels(hintType);
